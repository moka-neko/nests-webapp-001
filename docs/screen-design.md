...
| 項目 | 内容 |
|------|------|
| API | **`GET /api/v1/teachers/applications/{id}`（要 API 追加、§10 参照）** |
| 認証 | JWT Bearer |
| パス | `/teachers/:id` |
...
| ADM-05 | 先生詳細 | GET | `/api/v1/teachers/applications/{id}` | JWT（**要追加**） |
...
| 優先度 | 内容 | 理由 |
|--------|------|------|
| **高** | `GET /api/v1/teachers/applications/{id}` の追加 | ADM-05 専用詳細ページで必要。現状は Service 層に `findOneOrFail` はあるがエンドポイント未公開 |
| 中 | `GET /api/v1/students/applications/{id}` の追加 | 生徒詳細ページを分離する場合 |
| 低 | LINE コールバック後のフロントエンドリダイレクト | PUB-07 の UX 改善 |
...
| 2 | 先生応募管理（コア） | ADM-04, ADM-05（API 追加後）, ステータス変更 |
